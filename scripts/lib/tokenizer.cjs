...
/**
 * 对文本进行 jieba 分词 + 停用词过滤（去重，供 BM25 查询侧使用）
 * @param {string} text
 * @returns {string[]} 过滤后的词条列表
 */
...
/**
 * 对文本进行 jieba 分词 + 停用词过滤（不去重，保留词频信息，供索引构建侧使用）
 * @param {string} text
 * @returns {string[]} 保留顺序的分词结果列表
 */
function tokenizeAllFiltered(text) {
  const jieba = getJieba();
  const result = jieba.cut(text, false);
  const tokens = [];
  for (const token of result) {
